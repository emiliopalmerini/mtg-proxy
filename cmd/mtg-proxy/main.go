...
func main() {
	...
	flag.BoolVar(&artAll, "art-all", false, "include dithered art on every card")
	flag.Parse()
	if inputPath == "" {
	...
	for _, entry := range entries {
		...
		if artAll || entry.IsCommander {
			dc.ArtImages = make([]image.Image, len(c.Faces))
			for i, face := range c.Faces {
				...
				if artURL == "" {
					fmt.Fprintf(os.Stderr, "warning: no art available for %q\n", face.Name)
					continue
				}
