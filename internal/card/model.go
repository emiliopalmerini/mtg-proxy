...
func ParseManaCost(raw string) ManaCost { matches:=manaSymbolRegex.FindAllStringSubmatch(raw,-1); symbols:=make([]ManaSymbol,0,len(matches)); for _,m:=range matches{symbols=append(symbols,ManaSymbol{raw:m[1]})}; return ManaCost{symbols:symbols} }
...
type CardFace struct{Name CardName;ManaCost ManaCost;TypeLine TypeLine;OracleText OracleText;Stats *Stats;Loyalty *Loyalty;ArtCropURL string}
type Card struct{Faces []CardFace;ArtCropURL string}
...
type DeckEntry struct{Name CardName;Quantity Quantity;SetCode SetCode;CollectorNumber CollectorNumber;IsCommander bool}
type DeckCard struct{
	...
	// ArtImage is retained for compatibility with callers that provide one image.
